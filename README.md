...
live challenge lab (Iron Crown). 246 tests, green on Linux and Windows.
...
- **Ingestion** — nmap XML → hosts + services (monotonic host FSM); linPEAS / winPEAS →
  host-scoped privesc findings; **BloodHound** output → AD findings + chaining tags on the DC
...
- **Surface detection** — AI (Ollama, OpenAI-compat, chatbot, MCP, Gradio, A2A, vector DB) **and**
  ...
  live probes
- **Planner** — 57 declarative YAML technique rules across AI, traditional, cloud/k8s,
...
- **Actions layer** — 105 registered actions (`yhwach actions`) turn every playbook step into a
