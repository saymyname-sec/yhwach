...
The notes marked **⟳** are **auto-scaffolded from the world model** — run
`yhwach export-notes` (or the `yhwach_export_notes` MCP tool) and it regenerates
their tables straight from the DB, plus a per-host note. The generated blocks are
fenced with `<!-- yhwach:auto:<name> -->` markers; write prose *outside* those
fences and it survives a regenerate. The DB is the single source of truth for
structured facts (services, software, CVEs, principals, privileges, web paths,
shares); the vault is where you add the *why*, the payloads, and the evidence.
...
These are regenerated by `yhwach export-notes`; add prose around the fenced blocks, not inside.
...
- **Web** — per web-app: server, tech stack + versions, WAF, discovered dirs/paths (with the
  interesting ones — login/upload/admin/api/backup/source — starred), params, and vhosts/domains.
- **Users & Groups** — the AD identity layer split out from Credentials: principals (with flags
  like SPN / DONT_REQ_PREAUTH / adminCount), group memberships, and privileges/rights (local
  admin, DCSync, ESC1, …). Credentials.md stays the *secret* ledger; this is the *who + what
  they can do* map. Query the attack path between two nodes with `yhwach path --from X --to Y`.
- **Shares** — SMB/NFS shares per host with the current principal's access (read/write); a
  writable share is a foothold/relay lead, a readable one is loot.
